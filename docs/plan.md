...
- [x] 5-1. 정책 결정: **(b) 채택** — 지표 전용 연결과 관리(쓰기) 연결을 분리하는
  (a)안은 스코프 조합이 2가지로 늘어나 토큰 저장/재인증/`requireWriteScope` 검사
  로직 전반에 영향을 주는 큰 리팩터링이라, 이번에는 위험도가 낮은 UI 고지 강화만
  적용. 향후 실제 수요가 확인되면 (a)로 확장 검토.
- [x] 5-2. (a)안은 이번에 착수하지 않음 — 위 5-1 결정 참고.
- [x] 5-3. `PlatformConnectionsSection.tsx`의 연결 전 화면에 YouTube 전용 강조 안내
  박스(주황색, `AlertCircle` 아이콘)를 추가해 "영상 수정·삭제, 댓글 관리 권한까지
  함께 요청함"을 기존 회색 `scopeNote`보다 훨씬 눈에 띄게 고지. 연결 후에는
  표시하지 않음(이미 동의한 상태이므로).
- [ ] 5-4. 최종 정책 문서화는 UI 변경 수준에서는 별도 문서 반영이 크게 필요하지
  않다고 판단해 보류. (a)안으로 확장할 경우 `docs/api_spec.md`에 새 연결 옵션을
  반영해야 함.
