...
import { ArrowLeft, ArrowRight, Check, Instagram, Sparkles, Twitter, Youtube, AtSign } from 'lucide-react';
...
export const OnboardingHero: React.FC<OnboardingHeroProps> = ({ initialProfile, onRequestAuth, onSkipToDashboard }) => {
  ...
  // 개인/팀/기업 구분은 아직 화면을 바꾸지 않으므로 묻지 않는다. 저장된 값이 있으면 그대로 두고,
  // 없으면 individual로 보낸다 — 프로필 컬럼과 API는 나중에 쓸 수 있게 남겨둔다.
  const userType: UserType = initialProfile.userType || 'individual';
  ...
  const beginAuth = () => {
    onRequestAuth({ userType, selectedPlatforms });
  };
      ...
      <motion.div initial={{ opacity: 0, scale: 0.96, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.3 }} className="w-full max-w-xl glass-panel-elevated rounded-3xl p-5 sm:p-7 relative border border-white/80 shadow-xl">
        ...
        <div className="relative z-10 mb-6">
          <div className="flex items-center justify-between text-xs font-semibold text-slate-500 mb-2">
            <span className="flex items-center gap-1.5 text-indigo-600 font-bold"><span className="w-2 h-2 rounded-full bg-indigo-600 animate-pulse" />{step === 1 ? '1. 플랫폼 선택' : '2. 계정 인증'}</span>
            <span className="text-[11px] text-slate-400">{step} / 2</span>
          </div>
          <div className="grid grid-cols-2 gap-1.5">{[1, 2].map((item) => <div key={item} className={`h-1 rounded-full transition-all duration-300 ${item <= step ? 'bg-indigo-600' : 'bg-white/60'}`} />)}</div>
        </div>
        ...
        {step === 1 && (
          <div className="relative z-10 space-y-4">
            ...
            <div className="flex justify-end"><button type="button" onClick={() => setStep(2)} disabled={!selectedPlatforms.length} className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-6 py-2.5 rounded-xl bg-slate-900 text-white font-semibold text-xs hover:bg-slate-800 disabled:opacity-50">계정 등록<ArrowRight className="w-3.5 h-3.5" /></button></div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="relative z-10 space-y-4">
            ...
            <div className="rounded-xl bg-indigo-50/80 border border-indigo-100 px-3 py-2.5 text-xs text-indigo-800"><strong>{selectedPlatforms.length}개 플랫폼</strong> 선택 완료</div>
            ...
            <button type="button" onClick={() => setStep(1)} className="inline-flex items-center gap-1 text-xs text-slate-500 hover:text-slate-800"><ArrowLeft className="w-3.5 h-3.5" />플랫폼 다시 선택</button>
